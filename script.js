...
function atualizarCalcIndicadores() {
    ...
    // Status
    const temCC1    = _calcQtd['CC-1'] >= 1;
    const acimaTeto = totalVerba > TETO_VEREADOR + TOLERANCIA;
    const statusEl  = document.getElementById('calcStatus');
    const subEl     = document.getElementById('calcStatusSub');
    if (!temCC1) {
        statusEl.textContent = '✗ Inválido';
        statusEl.className   = 'calc-ind-valor calc-status-err';
        subEl.textContent    = 'CC-1 obrigatório não incluído';
    } else if (acimaTeto) {
        statusEl.textContent = '✗ Acima do teto';
        statusEl.className   = 'calc-ind-valor calc-status-err';
        subEl.textContent    = `Excedente: ${moeda(Math.abs(saldo))}`;
    } else if (totalVagas === 0) {
        statusEl.textContent = '— Vazio';
        statusEl.className   = 'calc-ind-valor';
        subEl.textContent    = 'Nenhum cargo selecionado';
    } else {
        statusEl.textContent = '✓ Válido';
        statusEl.className   = 'calc-ind-valor calc-status-ok';
        subEl.textContent    = `${totalVagas} cargo${totalVagas > 1 ? 's' : ''} selecionado${totalVagas > 1 ? 's' : ''}`;
    }
...
function gerarFormularioPDF() {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
    doc.setTextColor(0, 0, 128); // azul escuro igual ao formulário
    doc.text('FORMULÁRIO', PW / 2, MT + 5, { align: 'center' });
    doc.text('DE ESCOLHA DE COMPOSIÇÃO DE GABINETE PARLAMENTAR', PW / 2, MT + 11, { align: 'center' });
    ...
    // Câmara Municipal label
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7);
    doc.setTextColor(0);
    doc.text('Câmara Municipal de Curitiba', ML + 1, MT + 19);
    ...
    const nomeBoxW = 100;
    doc.rect(ML + lwLabel + 2, y - 5, nomeBoxW, 7);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9.5);
    doc.text(nomeVer, ML + lwLabel + 4, y - 0.5);
    ...
    const hVenc = 5.5;
    const colW  = [68, 22, 68, 22];
    const colX  = [ML, ML+68, ML+90, ML+158];
